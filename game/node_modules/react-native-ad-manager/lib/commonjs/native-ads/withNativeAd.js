...
var _TriggerableViewManager = require("./TriggerableViewManager");
...
const NativeAdView = _reactNative.UIManager.getViewManagerConfig(ComponentName) != null ? (0, _reactNative.requireNativeComponent)(ComponentName) : () => {
  // eslint-disable-next-line no-undef
  throw new Error(_Constants.LINKING_ERROR);
};
var _default = Component => class NativeAdWrapper extends _react.default.Component {
  constructor(props) {
    super(props);
    _defineProperty(this, "clickableChildrenNodeHandles", new Map());
    _defineProperty(this, "registerClickableChild", child => {
      const handle = (0, _reactNative.findNodeHandle)(child);
      if (!handle) {
        return;
      }
  ...
  componentDidUpdate(_, prevState) {
    ...
    if (clickableChildrenChanged) {
      const viewHandle = (0, _reactNative.findNodeHandle)(this);
      if (!viewHandle) {
        // Skip registration if the view is no longer valid.
        return;
      }
      _NativeAdsManager.NativeAdsManager.registerViewsForInteractionAsync(viewHandle, [...this.state.clickableChildren]).then(() => {
        // do nothing for now
      }).catch(() => {
        // do nothing for now
      });
  ...
  reloadAd() {
    _reactNative.UIManager.dispatchViewManagerCommand((0, _reactNative.findNodeHandle)(this), _reactNative.UIManager.getViewManagerConfig(ComponentName).Commands.reloadAd, []);
  }
  renderAdComponent(componentProps) {
    if (!this.state.nativeAd) {
      return null;
    }
    return /*#__PURE__*/_react.default.createElement(_TriggerableViewManager.TriggerableContext.Provider, {
      value: {
        register: this.registerClickableChild,
        unregister: this.unregisterClickableChild
      }
    }, /*#__PURE__*/_react.default.createElement(Component, _extends({}, componentProps, {
  ...
  render() {
    ...
    const _a = this.props,
      ...
      rest = (0, _utils.stripProperties)(_a, ['adsManager', 'onAdLoaded']);
    return /*#__PURE__*/_react.default.createElement(NativeAdView, {
